...
namespace Form_BanHang
{
    public partial class Form1 : Form
    {
        ...
        private void addData()
        {
            ...
            lstDanhSachMatHang.Items.Add("Phân thích thiết kế hệ thống");
        ...
        {
            string tenSPChon = lstDanhSachMatHang.SelectedItem.ToString();
            int flag = lstHangDatMua.FindString(tenSPChon);
            if (flag == -1)
            {
                lstHangDatMua.Items.Add(tenSPChon);
            }
            else MessageBox.Show("Sản phẩm đã được chọn");
        }
        ...
        private void lstHangDatMua_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (MessageBox.Show("Bạn có muốn xóa sản phẩm không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                == System.Windows.Forms.DialogResult.Yes)
            {
                lstDanhSachMatHang.Items.RemoveAt(index);
            }
        }
    }
}
